...
-- Fri 04 Oct 2024 09:13:31 AM -03
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Coleta` (
  ...
  `Tipo_Coleta` ENUM('1', '2', '3') NOT NULL COMMENT 'Capinas manuais, mecânicas ou químicas, dependendo da área',
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Culturas` (
  ...
  `NomeCultura` VARCHAR(100) NOT NULL COMMENT 'Tipos de cultura',
  ...
  `PropositoCultivo` INT(11) NOT NULL,
  `Tamanho` ENUM('1', '2', '3', '4', '5') NOT NULL COMMENT '1- Grande porte\n2- Médio porte\n3- Pequeno porte\n4-Leiras\n5-rasteiras\n',
  `Ciclo` INT NOT NULL COMMENT 'Ciclo de colheita em dias',
  `PropositoCultivo_idPropositoCultivo` INT NOT NULL,
  `TipoCultivo_idTipoCultivo` INT NOT NULL,
  `Cultura_Tratamento_idCultura_Tratamento` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Culturas_caracteristicas` (
  ...
  `Custo_Semente` DECIMAL(4,2) NOT NULL,
  `Pragas` MEDIUMTEXT NOT NULL,
  `Doencas` MEDIUMTEXT NOT NULL,
  `AnomaliaFisiologica` MEDIUMTEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Culturas_has_Requisitos` (
  ...
  `IDCulturas_has_Requisitoscol` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`IDCulturas_has_Requisitoscol`),
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Cultura_Tratamento` (
  ...
  `SoloPreparado` MEDIUMTEXT NOT NULL,
  `Tipos de solo` MEDIUMTEXT NOT NULL,
  `DensidadePlantio_Cultura` DECIMAL(2,2) NULL COMMENT 'Densidade de plantio da cultura específica',
  ...
  `cultivares e certificadas` MEDIUMTEXT NULL,
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Dados_Sis` (
  ...
  `Area_sistema` DECIMAL(10,5) NOT NULL COMMENT 'em hectares',
  `Densidade_Plantio` DECIMAL(10,5) NOT NULL,
  `CoberturaVegetalMedia` DECIMAL(10,5) NOT NULL,
  `RendimentoMedioCulturas` DECIMAL(10,5) NOT NULL COMMENT 'em toneladas por hectare',
  `TxRegeneracaoSistema` DECIMAL(10,5) NOT NULL COMMENT 'Taxa de regeneração do sistema em %',
  `Sis_Var_idSis_Var` INT(11) NOT NULL,
  PRIMARY KEY (`idDados`),
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`ReferenciaCulturas` (
  ...
  `Referencias_idReferencias` INT(11) NOT NULL,
  `Culturas_has_Requisitos_IDCulturas_has_Requisitoscol` VARCHAR(45) NOT NULL,
  ...
  INDEX `fk_ReferenciaCulturas_Culturas_has_Requisitos1_idx` (`Culturas_has_Requisitos_IDCulturas_has_Requisitoscol` ASC) VISIBLE,
  CONSTRAINT `fk_ReferenciaCulturas_Culturas_has_Requisitos1`
    FOREIGN KEY (`Culturas_has_Requisitos_IDCulturas_has_Requisitoscol`)
    REFERENCES `AgroFlorestaSIM`.`Culturas_has_Requisitos` (`IDCulturas_has_Requisitoscol`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Referencias` (
  `descricao_referencia` VARCHAR(1000) NOT NULL COMMENT 'Observaçãoes, como por exemplo o link de um site ou o nome do livro.\\\\\\\\n',
  `idReferencias` INT(11) NOT NULL AUTO_INCREMENT,
  `Tipo_referencia` ENUM('1', '2', '3', '4', '99') NOT NULL COMMENT 'especifique o tipo de referência:\\\\\\\\n1-Artigo Ciêntífico;\\\\\\\\n2-Livros;\\\\\\\\n3-Sites;\\\\\\\\n4-Trabalhos acadêmicos;\\\\\\\\n',
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Relac_Cultivo` (
  ...
  `Tipo` VARCHAR(45) NOT NULL COMMENT '1-QUem ajudamos\\\\\\\\n2- Quem nos ajuda',
  `Culturas_idPrincipal` INT(11) NOT NULL,
  `Culturas_idSecundaria` INT(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Requisitos` (
  ...
  `Tipo` ENUM('1', '2', '3', '4', '5') NOT NULL COMMENT '1- climatica\\\\\\\\n2- luminosa ',
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`Sis_Var` (
  ...
  `NumParcelas_Sis` DECIMAL(2,2) NOT NULL,
  `NumPlantasParc_Sis` DECIMAL(2,2) NOT NULL COMMENT 'numero de plantas por parcela',
  `Sis_EspacamentoPLantas` DECIMAL(2,2) NOT NULL COMMENT 'Espaçamento entre as plantas(em metrosO)',
  `Sis_Tempo` INT(11) NOT NULL COMMENT 'tempo em anos',
  `Culturas_idCulturas` INT(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `AgroFlorestaSIM`.`TipoCultivo` (
  ...
  `TIpo_Plantio` ENUM('1', '2', '3') NOT NULL COMMENT 'Tipo de plantio:\n1-Manual\n2-Mecânico\n3-outro',
